...
let stunnels : ('a, int, 'a Stunnel.t) Table.t ref = ref (Table.create capacity)
...
let unlocked_gc () = 
  if debug_enabled then begin
    ...
    let string_of_id id = 
      let stunnel = Table.find !stunnels id |> Scoped_dropable.borrow_exn in
  ...
  let all_ids = Table.fold (fun k _ acc -> k :: acc) !stunnels [] in
  ...
  Table.iter
    (fun idx stunnel ->
       ...
       let stunnel = Scoped_dropable.borrow_exn stunnel in
  ...
  if num_remaining > max_stunnel then begin
    ...
    List.iter
      (fun x -> 
         let stunnel = Table.find !stunnels x in
         debug "Expiring stunnel id %s since we have too many cached tunnels (limit is %d)" 
           (id_of_stunnel (Scoped_dropable.borrow_exn stunnel)) max_stunnel) oldest_ids;
    to_gc := !to_gc @ oldest_ids
  end;
  (* Disconnect all stunnels we wish to GC *)
  List.iter (fun id ->
      let s = Table.find !stunnels id in
      Scoped_dropable.release s) !to_gc;
  ...
  let stunnels' = Table.copy !stunnels in
  List.iter (fun idx -> Table.remove stunnels' idx) !to_gc;
...
let add (x: ('a, 'a Stunnel.t) Scoped_dropable.t) =
  let now = Unix.gettimeofday () in
  Mutex.execute m (fun () ->
      let idx = !counter in
      incr counter;
      Hashtbl.add !times idx now;
      Table.add !stunnels idx x;
      let x = Scoped_dropable.borrow_exn x in
...
let remove scope host port verified =
  ...
  Mutex.execute m
    (fun () ->
       ...
       match sorted with
       | (id, time) :: _ ->
         let stunnel = Table.find_and_move_exn scope !stunnels id in
         debug "Removing stunnel id %s (idle %.2f) from the cache"
           (id_of_stunnel (Scoped_dropable.borrow_exn stunnel)) (Unix.gettimeofday () -. time);
...
let flush () =
  Mutex.execute m 
    (fun () ->
       info "Flushing cache of all %d stunnels." (Table.length !stunnels);
       Table.reset !stunnels;
       Hashtbl.clear !times;
       Hashtbl.clear !index;
       info "Flushed!")
